...
c     |       to a 3-D velocity model below the bathymetry           |
c     |                                                              |
...
c
      ...
      read(5,*) ipol
      write(6,*) 'Enter depth below bathymetry and zmax of anomaly (km)'
      read(5,*) zamin,zamax
      ...
      vrel=ipol*vrel/100.
c
      open(12, file='bathymetry', form='unformatted',
     +       status='old')
      do 1300 j=1,ny
         read(12) (boundary(i,j),i=1,nx)
         do 1310 i=1,nx
...
c
      do 40 j=1,ny
         ...
         do 40 i=1,nx
           ...
           pan=1.+an
           nz1=nint((boundary(i,j)+zamin-zmin)/size)+1
           do 20 k=nz1,nz2
