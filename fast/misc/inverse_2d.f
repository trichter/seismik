...
c     I/O units:
c
c        10 -- input:  inverse 2D data grid
c
c        11 -- output:  input parameters
c
c        35 -- input:  parameters describing size of inverse 2D grid
c
c
c     ----------------------------------------------------------------
c
...
c    ***********************************************************
c
c      parameter(nnzmax=25000000, ndmax=250000)
      parameter(nnzmax=70000000, ndmax=200000)
...
c
      read(35,*) nx,ny,nz
c
      if(nx.gt.nximax.or.ny.ne.1.or.nz.gt.nzimax) then
        write(0,9)
9       format(/'***  model is too large or ny not equal 1  ***'/)
        stop
      end if
c
      nmodel=nx*nz
...
c
      if(interface.eq.1) then
        ...
        read(22) (ireg(i),i=1,nx)
      end if
c
      do 10 k=1,nz
         read(10) (imodel(i,k),i=1,nx)
10       read(16) (ibmodel(i,k),i=1,nx)
c
      read(44,*) nk,nobs
c
...
c
           do 40 i=2,nx-1
           if(interface.eq.1) then
             imax=max(ireg(i),ireg(i-1),ireg(i+1))
           else
             imax=0
           end if
...
c
           data(nconst)=
     +       smw*(-sx2*(imodel(i,k)-ibmodel(i,k))+
     +       sx*(imodel(i+1,k)-ibmodel(i+1,k))+
     +       sx*(imodel(i-1,k)-ibmodel(i-1,k)))/sn
           end if
40         continue
         else
c
c          horizontal flatness
c
           do 41 i=1,nx-1
           if(interface.eq.1) then
             imax=max(ireg(i),ireg(i+1))
           else
             imax=0
           end if
           if(imax+kstart.le.k.or.interface.eq.0) then
...
c
           data(nconst)=
     +       smw*sx*(-(imodel(i,k)-ibmodel(i,k))+
     +       (imodel(i+1,k)-ibmodel(i+1,k)))/sn
           end if
41         continue
         end if
400   continue
...
c
         ...
         column(nnz)=im3
c
         data(nconst)=(-szz*(imodel(i,k-1)-ibmodel(i,k-1))+
     +                 sz2*(imodel(i,k)-ibmodel(i,k)) 
     +                 -szz*(imodel(i,k+1)-ibmodel(i,k+1)))/sn
         end if
60    continue
c
      end if
      ...
      do 1100 k=1,nz
         ...
         do 110 i=1,nx
            ...
            if(imax+kstart.le.k.or.interface.eq.0) then
              im1=i+(k-1)*nx
c
              if(i.eq.1.or.k.eq.1.or.i.eq.nx.or.k.eq.nz) then
                if(sedge.gt.0.) then
                  nconst=nconst+1
                  sn=imodel(i,k)
                  nnz=nnz+1
                  ...
                  row(nnz)=nconst
                  column(nnz)=im1
                  data(nconst)=-anz(nnz)*(imodel(i,k)-
     +                                ibmodel(i,k))
                end if
              else
                if(sp.gt.0.) then
...
c
      do 190 k=1,nz
         do 210 i=1,nx
         im=i+(k-1)*nx
210      imodel(i,k)=dm(im)
         write(19) (imodel(i,k),i=1,nx)
190   continue
c
