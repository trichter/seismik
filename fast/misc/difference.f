...
c
      include 'ray.par'
c
      real vel1(nxmax),vel2(nxmax),maxdiff
      ...
      do 10 k=1,nz
         do 11 j=1,ny
           if(r4.eq.'r') then
             ...
             do 20 i=1,nx 
20              if(abs(vel1(i)-vel2(i)).gt.maxdiff) 
     +            maxdiff=abs(vel1(i)-vel2(i))
             write(12) (vel1(i)-vel2(i),i=1,nx)
           else
             read(10,end=999) (veli1(i),i=1,nx)
             read(11,end=999) (veli2(i),i=1,nx)
             do 21 i=1,nx 
21              if(abs(veli1(i)-veli2(i)).gt.imaxdiff) 
     +            imaxdiff=abs(veli1(i)-veli2(i))
             write(12) (veli1(i)-veli2(i),i=1,nx)
           end if
11       continue
         nk=nk+1
10    continue
...
200   if(r4.eq.'r') then
        if(maxdiff.eq.0.) then
          write(6,1)
1         format(/'***  the two files are the same  ***'/)
        else
          write(6,2) maxdiff
2         format(/'maximum difference between the two files: ',f12.5/)
        end if
      else
        if(imaxdiff.eq.0) then
          write(6,3)
