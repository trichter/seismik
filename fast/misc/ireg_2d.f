...
c     begin below an interface for 2D models
c
      include 'ray.par'
c
...
c
     ...
     +   nxi.ge.nxmax.or.nyi.gt.nymax.or.nzi.ge.nzmax) then
        write(0,8)
...
c
      ...
      do 20 i=1,nxi
         ...
         zimax=zmin
         do 30 ii=i1,i2
               if(boundary(ii,1).gt.zimax) zimax=boundary(ii,1)
30       continue
         ireg(i,1)=int((zimax-zmin)/zinc)+1    
20    continue
c
      do j=1,nyi
