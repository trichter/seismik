...
c      parameter(nnzmax=2000000, ndmax=3400)
      parameter(nnzmax=400000000, ndmax=5000000)
...
c      nr=nk/nwrite
      nr=nk
c      nl=nk-nr*nwrite
c      if(nl.gt.0) nr=nr+1
c
      do 20 j=1,nr
c         is=(j-1)*nwrite
c         read(12) (anz(is+i),i=1,nwrite)
c         read(14) (row(is+i),i=1,nwrite)
c         read(15) (column(is+i),i=1,nwrite)
         read(12) anz(j)
         read(14) row(j)
         read(15) column(j)
c         print*,'INVERSE',j,anz(j),row(j),column(j)
20    continue
c
      do 30 i=1,nobs
30       read(13) data(i)
...
40         continue
c
c          horizontal smoothness (1D operator at model back and front)
c
           do 410 j=1,ny,ny-1
              do 410 i=2,nx-1
           if(interface.eq.1) then
...
c
           data(nconst)=
     +       smw*(-sx2*(imodel(i,j,k)-ibmodel(i,j,k))+
     +       sx*(imodel(i+1,j,k)-ibmodel(i+1,j,k))+
     +       sx*(imodel(i-1,j,k)-ibmodel(i-1,j,k)))/sn
           end if
410        continue
c
c          horizontal smoothness (1D operator at model left and right)
c
           do 420 i=1,nx,nx-1
              do 420 j=2,ny-1
           if(interface.eq.1) then
           imax=max(ireg(i,j),ireg(i,j-1),ireg(i,j+1))
           else
           imax=0
           end if
           if(imax+kstart.le.k.or.interface.eq.0) then
           im1=i+(j-1)*nx+(k-1)*nx*ny
           im2=i+(j-2)*nx+(k-1)*nx*ny
           im3=i+(j)*nx+(k-1)*nx*ny
           nconst=nconst+1
c           
           sn=imodel(i,j,k)
           nnz=nnz+1
...
c      parameter(nnzmax=200000, ndmax=3400)
      parameter(nnzmax=400000000, ndmax=1000000)
      parameter(nsconst=4*nximax*nyimax*nzimax,
     +          nszero=9*nximax*nyimax*nzimax,
     +          nmmax=nximax*nyimax*nzimax)
c
...
c
      integer row(nnzmax+nszero),column(nnzmax+nszero)
      real anz(nnzmax+nszero),u(m),v(n),sum(1:nmmax)
c
      common /blk11/ anz
      common /blk22/ row
      common /blk33/ column
