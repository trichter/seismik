...
def fit_legs(x2, x, t):
    ...
    for i in range(len(x2)-1):
        ...
        mc, err = linear_fit_through(point, x[ind], t[ind])
        sum_err += err
        point = (x2[i+1], x2[i+1] * mc[0] + mc[1])
        mcs.append(mc)
    return mcs, x2, sum_err


def func(x2, x, t):
    if any(x2[i] >= x2[i+1] for i in range(len(x2) - 1)):
        return 100000
    _, _, sum_err = fit_legs(x2, x, t)
    return sum_err
...
def fit_ndmodel(n, x, t):
    ...
    x2 = brute(func, ranges, args=(x, t), workers=-1)
...
def tt2model(mcs):
    ...
    for n in range(N - 1):
        ...
        h.append(hn)
    for i in range(N-1):
        print(f'  depth up to {h[i]:.1f}m: v={v[i]/1000:.2f}km/s')
    h2 = h[-1] if len(h)>0 else 0
    print(f'  below       {h2:.1f}m: v={v[-1]/1000:.2f}km/s')
    return h, v
...
def fit_model(x, t, n=None, ttpicks=None, mode=None):
    ...
    if ttpicks and mode == 't':
        ...
        x2, _ = zip(*ttpicks)
        mcs, x2, _ = fit_legs(x2, x, t)
    else:
        assert n is not None
        if n == 1:
            mcs, x2, _ = fit_legs([], x, t)
        else:
            mcs, x2, _ = fit_ndmodel(n, x, t)
